...
export function initLiveChatSidebar() {
  // 🛑 Removed initial DOM queries here. We will query inside the rendering functions
  // as they are called later, ensuring we get the live DOM node.
  const main = document.getElementById("mainContent");

  // We only check for 'main' as it's required for the 'item.addEventListener' later.
  if (!main) {
...
  let activeChatUserId = null;
  ...
  // NOTE: currentUserInfo is *not* initialized here, but we will ensure it is queried
  // before being written to in checkSession.
...
  async function checkSession() {
    const currentUserInfo = document.getElementById("currentUserInfo");
    if (!currentUserInfo) return; // Exit if the layout wasn't fully initialized
...
  function renderUsersList(users) {
    // ✅ PATCH 1: Re-query the usersList element to ensure we target the live DOM node.
    const usersList = document.getElementById("usersList");
    if (!usersList) return; 
...
  function refreshOnlineIndicators() {
    // ✅ PATCH 2: Re-query the usersList element here as well.
    const usersList = document.getElementById("usersList");
    if (!usersList) return;
...
      unreadCounts.set(senderId, prev + 1);
      if (lastUsers.length) {
        renderUsersList(lastUsers); // This call now uses the patched function
      }
    }
...
    refreshOnlineIndicators(); // This call now uses the patched function
